...
class Nearby_Recompute_Job {
    ...
    public function process_nearby_data($origin_id, $type, $candidates) {
        try {
            ...
            if ($type === 'poi') {
                ...
                $meta_key = '_db_nearby_cache_rv_foot';
            } else {
            ...
            // ORS dokumentace dovoluje maximálně jeden matrix dotaz (1 origin + 49 destinací)
            // za minutu, proto vždy zpracujeme všechny kandidáty v jediném požadavku.
            $normalized_candidates = $this->normalize_matrix_candidates($candidates);
            $chunk_result = $this->process_chunk($normalized_candidates, $orsKey, $provider, $profile, $origin_lat, $origin_lng, $origin_id, 0);
            ...
            if (!empty($orsKey) && $provider === 'ors') {
                $iso_status = $this->fetch_and_cache_isochrones($origin_id, $origin_lat, $origin_lng, $orsKey);

                if (empty($iso_status['success'])) {
                    $error_message = !empty($iso_status['error']) ? (string)$iso_status['error'] : 'Nepodařilo se uložit isochrony';

                    if (!empty($iso_status['retry_after'])) {
                        $error_message .= ' (zkusit znovu kolem ' . date('H:i', (int)$iso_status['retry_after']) . ')';
                    }

                    return array(
                        'success' => false,
                        'error' => $error_message,
                        'retry_after' => $iso_status['retry_after'] ?? null,
                        'isochrones' => $iso_status
                    );
                }

                if (!empty($iso_status['called'])) {
                    $iso_calls++;
                }
            }
    ...
    public function recompute_nearby_for_origin($origin_id, $type) {
        ...
        try {
            ...
            // 2) ORS Matrix jedním požadavkem (1 origin + až 49 destinací)
            $normalized_candidates = $this->normalize_matrix_candidates($candidates);
            $matrix_result = $this->process_chunk($normalized_candidates, $orsKey, $provider, $profile, $lat, $lng, $origin_id, 0);

            if (empty($matrix_result['success'])) {
                $retry_at = isset($matrix_result['retry_after']) ? (int)$matrix_result['retry_after'] : null;
                $retry_delay = $retry_at ? max(0, $retry_at - time()) : null;

                $this->debug_log('[Matrix] single request failed', [
                    'origin_id' => $origin_id,
                    'type' => $type,
                    'error' => $matrix_result['error'] ?? null,
                ...
                $this->write_cache(
                    ...
                    $total,
                    null,
                    $matrix_result['error'] ?? 'matrix_failed',
                    $retry_delay
                );

                if ($retry_at && !wp_next_scheduled('db_nearby_recompute', [$origin_id, $type])) {
                    wp_schedule_single_event($retry_at, 'db_nearby_recompute', [$origin_id, $type]);
                }

                return;
            }

            $items = $matrix_result['items'];
            usort($items, fn($a, $b) => ($a['duration_s'] <=> $b['duration_s']));
            ...
            $matrix_calls = ($provider === 'ors') ? 1 : 0;
            $iso_calls = 0;
            $iso_status = null;

            if (!empty($orsKey) && $provider === 'ors') {
                $iso_status = $this->fetch_and_cache_isochrones($origin_id, $lat, $lng, $orsKey);

                if (empty($iso_status['success'])) {
                    $this->debug_log('[Isochrones] recompute warning', array(
                        'origin_id' => $origin_id,
                        'status' => $iso_status['status'] ?? 'unknown',
                        'error' => $iso_status['error'] ?? null,
                        'retry_after' => $iso_status['retry_after'] ?? null
                    ));

                    if (!empty($iso_status['retry_after']) && !wp_next_scheduled('db_nearby_recompute', [$origin_id, $type])) {
                        wp_schedule_single_event((int)$iso_status['retry_after'], 'db_nearby_recompute', [$origin_id, $type]);
                    }
                }
            ...
            $processing_time = round(microtime(true) - $processing_start, 3);
            $this->track_processed_location(
                $origin_id,
                $type,
                $candidates,
                array('matrix' => $matrix_calls, 'isochrones' => $iso_calls),
                $processing_time
            );
    ...
    /**
     * Načíst a uložit isochrones data současně s nearby daty.
     *
     * @return array{
     *     success: bool,
     *     called: bool,
    ...
    private function fetch_and_cache_isochrones($origin_id, $lat, $lng, $orsKey) {
        ...
        try {
            $profile = 'foot-walking';
            $ranges = array(552, 1124, 1695); // Realistických ~10/20/30 min pěšky
            ...
            if ($existing_cache && !empty($existing_cache)) {
                ...
                if ($payload && isset($payload['geojson']['features']) && !empty($payload['geojson']['features']) && empty($payload['error'])) {
                    $ttl_days = isset($payload['ttl_days']) ? (int)$payload['ttl_days'] : 30;
                    $computed_at = isset($payload['computed_at']) ? strtotime($payload['computed_at']) : 0;
                    if ($computed_at > 0 && (time() - $computed_at) < ($ttl_days * DAY_IN_SECONDS)) {
                        $result['success'] = true;
                        $result['cached'] = true;
                        $result['status'] = 'cached_valid';
                        return $result;
                    }
                }
            }
            ...
            if (!$minute_check['allowed']) {
                $wait = isset($minute_check['wait_seconds']) ? (int)$minute_check['wait_seconds'] : 60;
                $retry_epoch = time() + max(1, $wait);

                $this->debug_log('[Isochrones] token bucket blocked', array(
                    'origin_id' => $origin_id,
                    'wait_seconds' => $wait,
                ...
                $result['status'] = 'rate_limited_local';
                $result['error'] = 'Lokální minutový limit pro isochrony';
                $result['retry_after'] = $retry_epoch;
                return $result;
            }

            $body = array(
                'locations' => array(array((float)$lng, (float)$lat)),
                'range' => $ranges,
                'range_type' => 'time'
            );

            $this->debug_log('[Isochrones] sending request', array(
                'origin_id' => $origin_id,
                'ranges' => $ranges,
                'tokens_after' => $minute_check['tokens_after'] ?? null
            ));
            ...
            if (is_wp_error($response)) {
                ...
                $this->debug_log('[Isochrones] wp_remote_post error', array(
                    'origin_id' => $origin_id,
                    'error' => $error_message
                ));
            ...
            $this->debug_log('[Isochrones] response received', array(
                'origin_id' => $origin_id,
                'http_code' => $http_code,
            ...
            if ($http_code === 401 || $http_code === 403) {
                ...
                $result['error'] = 'ORS API key byl odmítnut pro isochrony';
                return $result;
            }

            if ($http_code === 429) {
                $wait = $retry_after_header !== null ? max(1, $retry_after_header) : 60;
                $retry_epoch = time() + $wait;

                $this->set_isochrones_error($origin_id, $profile, 'rate_limited', 'Rate limited', $wait);
                $result['status'] = 'rate_limited_remote';
                $result['error'] = 'ORS vrátil 429 Rate Limited';
                $result['retry_after'] = $retry_epoch;
                return $result;
            }
            ...
            if ($http_code !== 200) {
                ...
                $result['error'] = "Neočekávaná odpověď ORS: HTTP {$http_code} ({$body_excerpt})";
                return $result;
            }

            if (!is_array($data) || empty($data)) {
                $this->set_isochrones_error($origin_id, $profile, 'invalid_payload', 'Prázdná odpověď');
                $result['status'] = 'invalid_payload';
                $result['error'] = 'ORS isochrones vrátil prázdná data';
                return $result;
            }
            ...
            $payload = array(
                ...
                'profile' => $profile,
                'ranges_s' => $ranges,
                'center' => array((float)$lng, (float)$lat),
                'geojson' => $data,
            ...
            update_post_meta($origin_id, $meta_key, wp_json_encode($payload));

            $this->debug_log('[Isochrones] cached', array(
                'origin_id' => $origin_id,
                'features' => count($data['features'] ?? array()),
                'ratelimit_remaining' => $remaining_header
            ));
            ...
            $this->debug_log('[Isochrones] exception', array(
                'origin_id' => $origin_id,
                'error' => $e->getMessage()
            ));
    ...
    private function process_ors_chunk($chunk, $orsKey, $profile, $origin_lat, $origin_lng, $origin_id = null, $chunk_index = null) {
        ...
        if (!$minute_check['allowed']) {
            ...
            $retry_at = $wait ? time() + (int)$wait : null;
