<?php
/**
 * Charging discovery queue manager – handles DB backed queue for asynchronous
 * discovery of Google Places / Open Charge Map identifiers.
 */
...
class Charging_Discovery_Queue_Manager {
    private $table;

    public function __construct() {
        global $wpdb;
        $this->table = $wpdb->prefix . 'db_charging_discovery_queue';
    ...
    public function enqueue(int $stationId, int $priority = 0): bool {
        global $wpdb;
        $this->ensure_table_exists();
        $row = $wpdb->get_row($wpdb->prepare("SELECT id, status FROM {$this->table} WHERE station_id=%d LIMIT 1", $stationId));
        if ($row) {
    ...
    public function mark_failed_or_retry(int $id, string $error): void {
        ...
        $wpdb->update(
            $this->table,
            array('status' => $status, 'attempts' => $attempts, 'error_message' => $error),
            array('id' => $id),
            array('%s', '%d', '%s'),
            array('%d')
        );
    ...
    public function move_to_review(int $id, string $provider, string $matchedId, float $score): void {
        global $wpdb;
        $this->ensure_table_exists();
        $wpdb->update(
            $this->table,
